...
public final class BlockBackgroundNode<R extends BasicRenderContext> extends Node$a<R> implements Spoilerable {
    ...
    public void render(SpannableStringBuilder builder, R renderContext) {
        ...
        if (getIsRevealed()) {
            spoilerColorRes = ColorCompat.getThemedColor(context, 2130970275);
        } else {
            if (!(renderContext instanceof SpoilerNode$RenderContext)) {
                renderContext = null;
            }
            SpoilerNode$RenderContext spoilerNode$RenderContext = (SpoilerNode$RenderContext) renderContext;
            spoilerColorRes = spoilerNode$RenderContext != null ? spoilerNode$RenderContext.getSpoilerColorRes() : ColorCompat.getThemedColor(context, 2130970291);
        }
        builder.setSpan(new BlockBackgroundSpan(spoilerColorRes, ColorCompat.getThemedColor(context, 2130970276), DimenUtils.dpToPixels(1), DimenUtils.dpToPixels(4), this.inQuote ? BlockQuoteNode.Companion.getTOTAL_LEFT_MARGIN() : 0), length, builder.length(), 33);
