...
public class EclipseProductLocationProvider implements IdeLocationProvider {
    ...
    public void findIdes(List<IdeLocation> locations, List<CorruptedIdeLocationException> problems) {
        switch ($SWITCH_TABLE$lombok$installer$OsUtils$OS()[OsUtils.getOS().ordinal()]) {
            case 1:
                new EclipseProductLocationProvider$MacFinder(this).findEclipse(locations, problems);
                break;
            case 2:
                new EclipseProductLocationProvider$WindowsFinder(this).findEclipse(locations, problems);
                break;
            case 3:
            default:
                new EclipseProductLocationProvider$UnixFinder(this).findEclipse(locations, problems);
                break;
        }
    }
