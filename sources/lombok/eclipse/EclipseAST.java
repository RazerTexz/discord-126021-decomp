...
public class EclipseAST extends AST<EclipseAST, EclipseNode, ASTNode> {
    ...
    private final List<EclipseAST$ParseProblem> queuedProblems;
    private final CompilationUnitDeclaration compilationUnitDeclaration;
    private boolean completeParse;
    private static /* synthetic */ int[] $SWITCH_TABLE$lombok$core$AST$Kind;

    @Override // lombok.core.AST
    protected /* bridge */ /* synthetic */ LombokNode buildTree(ASTNode aSTNode, AST$Kind aST$Kind) {
        return buildTree2(aSTNode, aST$Kind);
    }
    ...
    private static URI getAbsoluteFileLocation0(CompilationUnitDeclaration ast) {
        ...
        if (!skipEclipseWorkspaceBasedFileResolver) {
            try {
                try {
                    return EclipseAST$EclipseWorkspaceBasedFileResolver.resolve(fileName);
    ...
    private void propagateProblems() {
        ...
        for (EclipseAST$ParseProblem problem : this.queuedProblems) {
            problem.addToCompilationResult();
        }
        this.queuedProblems.clear();
    }

    void addProblem(EclipseAST$ParseProblem problem) {
    ...
    public static void addProblemToCompilationResult(char[] fileNameArray, CompilationResult result, boolean isWarning, String message, int sourceStart, int sourceEnd) {
        try {
            EclipseAST$EcjReflectionCheck.addProblemToCompilationResult.invoke(null, fileNameArray, result, Boolean.valueOf(isWarning), message, Integer.valueOf(sourceStart), Integer.valueOf(sourceEnd));
        } catch (IllegalAccessException e) {
            throw Lombok.sneakyThrow(e);
        } catch (NoClassDefFoundError unused) {
        } catch (NullPointerException e2) {
            if (!"false".equals(System.getProperty("lombok.debug.reflection", "false"))) {
                e2.initCause(EclipseAST$EcjReflectionCheck.problemAddProblemToCompilationResult);
                throw e2;
            }
    ...
    public static Annotation[] getTopLevelTypeReferenceAnnotations(TypeReference tr) {
        Annotation[][] annss;
        Method m = EclipseAST$EcjReflectionCheck.typeReferenceGetAnnotationsOnDimensions;
        if (m == null) {
            return null;
        }
        try {
        ...
        try {
            Field f = EclipseAST$EcjReflectionCheck.typeReferenceAnnotations;
