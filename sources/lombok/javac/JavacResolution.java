...
public class JavacResolution {
    ...
    public Map<JCTree, JCTree> resolveMethodMember(JavacNode node) {
        ...
        try {
            JavacResolution$EnvFinder finder = new JavacResolution$EnvFinder(node.getContext());
            while (!stack.isEmpty()) {
    ...
    public static Type ifTypeIsIterableToComponent(Type type, JavacAST ast) {
        ...
        return iterableParams.isEmpty() ? syms.objectType : JavacResolution$ReflectiveAccess.Types_upperBound(types, (Type) iterableParams.head);
    }

    public static JCTree$JCExpression typeToJCTree(Type type, JavacAST ast, boolean allowVoid) throws JavacResolution$TypeNotConvertibleException {
    ...
    private static JCTree$JCExpression typeToJCTree(Type type, JavacAST ast, boolean allowCompound, boolean allowVoid, boolean allowCapture) throws JavacResolution$TypeNotConvertibleException {
        Type type0;
        int dims = 0;
        Type type2 = type;
        while (true) {
        ...
        JCTree$JCArrayTypeTree jCTree$JCArrayTypeTreeTypeToJCTree0 = typeToJCTree0(type0, ast, allowCompound, allowVoid, allowCapture);
        while (dims > 0) {
            jCTree$JCArrayTypeTreeTypeToJCTree0 = ast.getTreeMaker().TypeArray(jCTree$JCArrayTypeTreeTypeToJCTree0);
            dims--;
        }
        return jCTree$JCArrayTypeTreeTypeToJCTree0;
    }

    private static Iterable<? extends Type> concat(Type t, Collection<? extends Type> ts) {
        return t == null ? ts : new JavacResolution$1(ts, t);
    }
    ...
    private static JCTree$JCExpression typeToJCTree0(Type type, JavacAST ast, boolean allowCompound, boolean allowVoid, boolean allowCapture) throws JavacResolution$TypeNotConvertibleException {
        Type lower;
        Type upper;
        String qName;
        int level;
        ...
        if (type.isErroneous()) {
            throw new JavacResolution$TypeNotConvertibleException("Type cannot be resolved");
        }
        Symbol$TypeSymbol symbol = type.asElement();
        List<Type> generics = type.getTypeArguments();
        JCTree$JCIdent jCTree$JCIdentTypeToJCTree0 = null;
        if (symbol == null) {
            throw new JavacResolution$TypeNotConvertibleException("Null or compound type");
        }
        if (symbol.name.length() == 0) {
            ...
            throw new JavacResolution$TypeNotConvertibleException("Anonymous inner class");
        }
        ...
        if (symbol.isLocal()) {
            ...
            jCTree$JCIdentTypeToJCTree0 = typeToJCTree0(type.getEnclosingType(), ast, false, false, false);
            qName = symbol.getSimpleName().toString();
        } else {
            qName = symbol.getQualifiedName().toString();
        }
        if (qName.isEmpty()) {
            throw new JavacResolution$TypeNotConvertibleException("unknown type");
        }
        if (qName.startsWith("<")) {
            throw new JavacResolution$TypeNotConvertibleException(qName);
        }
        String[] baseNames = qName.split("\\.");
        int i = 0;
        if (jCTree$JCIdentTypeToJCTree0 == null) {
            jCTree$JCIdentTypeToJCTree0 = maker.Ident(ast.toName(baseNames[0]));
            i = 1;
        }
        while (i < baseNames.length) {
            jCTree$JCIdentTypeToJCTree0 = maker.Select(jCTree$JCIdentTypeToJCTree0, ast.toName(baseNames[i]));
            i++;
        }
        return genericsToJCTreeNodes(generics, ast, jCTree$JCIdentTypeToJCTree0);
    }
    ...
    private static JCTree$JCExpression genericsToJCTreeNodes(List<Type> generics, JavacAST ast, JCTree$JCExpression rawTypeNode) throws JavacResolution$TypeNotConvertibleException {
    ...
    private static JCTree$JCExpression primitiveToJCTree(TypeKind kind, JavacTreeMaker maker) throws JavacResolution$TypeNotConvertibleException {
        switch ($SWITCH_TABLE$javax$lang$model$type$TypeKind()[kind.ordinal()]) {
            ...
            case 19:
            default:
                throw new JavacResolution$TypeNotConvertibleException("Nulltype");
        }
    }
