...
public class HandleConstructor$HandleRequiredArgsConstructor extends JavacAnnotationHandler<RequiredArgsConstructor> {
    ...
    public void handle(AnnotationValues<RequiredArgsConstructor> annotation, JCTree$JCAnnotation ast, JavacNode annotationNode) {
        ...
        if (HandleConstructor.checkLegality(typeNode, annotationNode, NAME)) {
            ...
            this.handleConstructor.generateConstructor(typeNode, level, onConstructor, HandleConstructor.findRequiredFields(typeNode), false, staticName, HandleConstructor$SkipIfConstructorExists.NO, annotationNode);
        }
    }
}
