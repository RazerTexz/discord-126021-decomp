...
public class HandleBuilder extends JavacAnnotationHandler<Builder> {
    ...
    public void handle(AnnotationValues<Builder> annotation, JCTree$JCAnnotation ast, JavacNode annotationNode) {
        ...
        HandleBuilder$BuilderJob job = new HandleBuilder$BuilderJob();
        ...
        if (HandlerUtil.checkName("buildMethodName", job.buildMethodName, annotationNode)) {
            ...
            if (parent.get() instanceof JCTree$JCClassDecl) {
                ...
                if (job.toBuilder) {
                    ...
                    for (JCTree$JCTypeParameter tp : tpOnMethod) {
                        ...
                        while (it.hasNext()) {
                            JCTree$JCIdent jCTree$JCIdent = (JCTree$JCExpression) it.next();
                            idx++;
                            if ((jCTree$JCIdent instanceof JCTree$JCIdent) && jCTree$JCIdent.name == tp.name) {
                                pos = idx;
                            }
                        }
            ...
            if (fillParametersFrom != null) {
                for (JavacNode param : fillParametersFrom.down()) {
                    if (param.getKind() == AST$Kind.ARGUMENT) {
                        HandleBuilder$BuilderFieldData bfd2 = new HandleBuilder$BuilderFieldData();
            ...
            for (HandleBuilder$BuilderFieldData bfd4 : job.builderFields) {
    ...
    static String returnTypeToBuilderClassName(JavacNode annotationNode, JCTree$JCClassDecl td, JCTree$JCExpression returnType, List<JCTree$JCTypeParameter> typeParams) {
        ...
        if (returnType instanceof JCTree$JCFieldAccess) {
            ...
            JCTree$JCFieldAccess jCTree$JCFieldAccess = ((JCTree$JCTypeApply) returnType).clazz;
            if (jCTree$JCFieldAccess instanceof JCTree$JCFieldAccess) {
                replStr = jCTree$JCFieldAccess.name.toString();
            } else if (jCTree$JCFieldAccess instanceof JCTree$JCIdent) {
                replStr = ((JCTree$JCIdent) jCTree$JCFieldAccess).name.toString();
            }
        }
    ...
    private JCTree$JCMethodDecl generateToBuilderMethod(HandleBuilder$BuilderJob job, List<JCTree$JCTypeParameter> typeParameters, String prefix) {
        ...
        JCTree$JCMethodInvocation jCTree$JCMethodInvocationNewClass = maker.NewClass(null, List.nil(), JavacHandlerUtil.namePlusTypeParamsToTypeReference(maker, job.parentType, job.toName(job.builderClassName), !job.isStatic, job.builderTypeParams), List.nil(), null);
        ListBuffer<JCTree$JCStatement> preStatements = null;
        ListBuffer<JCTree$JCStatement> statements = new ListBuffer<>();
        for (HandleBuilder$BuilderFieldData bfd : job.builderFields) {
            String setterPrefix = prefix.isEmpty() ? job.oldFluent ? "" : "set" : prefix;
            ...
            if (bfd.singularData == null) {
                ...
                jCTree$JCMethodInvocationNewClass = maker.Apply(List.nil(), maker.Select(jCTree$JCMethodInvocationNewClass, setterName), List.of(arg));
            } else {
        ...
        if (!statements.isEmpty()) {
            ...
            statements.prepend(maker.VarDef(maker.Modifiers(16L), job.toName(BUILDER_TEMP_VAR), tempVarType, jCTree$JCMethodInvocationNewClass));
            statements.append(maker.Return(maker.Ident(job.toName(BUILDER_TEMP_VAR))));
        } else {
            statements.append(maker.Return(jCTree$JCMethodInvocationNewClass));
        }
        if (preStatements != null) {
    ...
    private JCTree$JCMethodDecl generateCleanMethod(HandleBuilder$BuilderJob job) {
        JavacTreeMaker maker = job.getTreeMaker();
        ListBuffer<JCTree$JCStatement> statements = new ListBuffer<>();
        for (HandleBuilder$BuilderFieldData bfd : job.builderFields) {
    ...
    static JCTree$JCVariableDecl generateReceiver(HandleBuilder$BuilderJob job) {
        JCTree$JCLiteral jCTree$JCLiteralNewArray;
        ...
        if (mandatories.size() == 1) {
            jCTree$JCLiteralNewArray = maker.Literal(mandatories.get(0));
        } else {
            ...
            jCTree$JCLiteralNewArray = maker.NewArray(null, List.nil(), elems);
        }
        JCTree$JCAnnotation recvAnno = maker.Annotation(JavacHandlerUtil.genTypeRef(job.builderType, CheckerFrameworkVersion.NAME__CALLED), List.of(jCTree$JCLiteralNewArray));
    ...
    private JCTree$JCMethodDecl generateBuildMethod(HandleBuilder$BuilderJob job, Name staticName, JCTree$JCExpression returnType, List<JCTree$JCExpression> thrownExceptions, boolean addCleaning) {
        ...
        if (staticName == null) {
            ...
            JCTree$JCMethodInvocation jCTree$JCMethodInvocationApply = maker.Apply(typeParams.toList(), maker.Select(callee, staticName), args.toList());
            if ((returnType instanceof JCTree$JCPrimitiveTypeTree) && Javac.CTC_VOID.equals(JavacTreeMaker$TypeTag.typeTag((JCTree) returnType))) {
                statements.append(maker.Exec(jCTree$JCMethodInvocationApply));
            } else {
                statements.append(maker.Return(jCTree$JCMethodInvocationApply));
            }
        }
        JCTree$JCBlock body = maker.Block(0L, statements.toList());
    ...
    public static JCTree$JCMethodDecl generateDefaultProvider(Name methodName, JavacNode fieldNode, List<JCTree$JCTypeParameter> params) {
        ...
        JCTree$JCReturn jCTree$JCReturnReturn = maker.Return(field.init);
        field.init = null;
        JCTree$JCBlock body = maker.Block(0L, List.of(jCTree$JCReturnReturn));
    ...
    public JCTree$JCMethodDecl generateBuilderMethod(HandleBuilder$BuilderJob job) {
        JCTree$JCNewClass jCTree$JCNewClassNewClass;
        JavacTreeMaker maker = job.getTreeMaker();
        ListBuffer<JCTree$JCExpression> typeArgs = new ListBuffer<>();
        ...
        if (job.isStatic) {
            jCTree$JCNewClassNewClass = maker.NewClass(null, List.nil(), JavacHandlerUtil.namePlusTypeParamsToTypeReference(maker, job.parentType, job.toName(job.builderClassName), false, job.typeParams), List.nil(), null);
        } else {
            jCTree$JCNewClassNewClass = maker.NewClass(null, List.nil(), JavacHandlerUtil.namePlusTypeParamsToTypeReference(maker, null, job.toName(job.builderClassName), false, job.typeParams), List.nil(), null);
            jCTree$JCNewClassNewClass.encl = maker.Ident(job.toName("this"));
        }
        JCTree$JCBlock body = maker.Block(0L, List.of(maker.Return(jCTree$JCNewClassNewClass)));
    ...
    public void generateBuilderFields(HandleBuilder$BuilderJob job) {
        ...
        for (int i = len - 1; i >= 0; i--) {
            HandleBuilder$BuilderFieldData bfd = job.builderFields.get(i);
            if (bfd.singularData != null && bfd.singularData.getSingularizer() != null) {
    ...
    public void makePrefixedSetterMethodsForBuilder(HandleBuilder$BuilderJob job, HandleBuilder$BuilderFieldData bfd, String prefix) {
    ...
    private void makePrefixedSetterMethodForBuilder(HandleBuilder$BuilderJob job, HandleBuilder$BuilderFieldData bfd, boolean deprecate, String prefix) {
    ...
    public JavacNode makeBuilderClass(HandleBuilder$BuilderJob job) {
        JavacTreeMaker maker = job.getTreeMaker();
        int modifiers = JavacHandlerUtil.toJavacModifier(job.accessOuters);
        if (job.isStatic) {
            modifiers |= 8;
