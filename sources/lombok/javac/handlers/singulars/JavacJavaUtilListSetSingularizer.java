...
abstract class JavacJavaUtilListSetSingularizer extends JavacJavaUtilSingularizer {
    ...
    protected JCTree$JCStatement generateClearStatements(JavacTreeMaker maker, JavacSingularsRecipes$SingularData data, JavacNode builderType) {
        ...
        JCTree$JCFieldAccess jCTree$JCFieldAccessSelect = maker.Select(maker.Ident(builderType.toName("this")), data.getPluralName());
        return maker.If(maker.Binary(Javac.CTC_NOT_EQUAL, jCTree$JCFieldAccessSelect, maker.Literal(Javac.CTC_BOT, null)), maker.Exec(maker.Apply(jceBlank, maker.Select(maker.Select(maker.Ident(builderType.toName("this")), data.getPluralName()), builderType.toName("clear")), jceBlank)), null);
    }
